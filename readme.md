...
|                          | A4.5   | AOM3A1    | AOM3A2   | BP10      | CF2.2   | CF3.0     | CM20   | novelai   | <font color=red>BPXL0.3.1</font>  | <font color=red>CFXL1.0</red>   |
|:-------------------------|:-------|:----------|:---------|:----------|:--------|:----------|:-------|:----------|:------------|:----------|
...
- 准确率的意思是，假如我用`AOM3A1`模型，生成了`n`张包含`m`个tag的图片，即每张的prompt中包含`m`个随机的tag，然后用ML-Danbooru检测这`n`张图片，每张图片中能被预测出和prompt中相同的tag的总和为`k`，那么`AOM3A1`×`n tags`的准确率就是`k/(n*m)`。
